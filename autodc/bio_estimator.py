...
class Classifier(BaseEstimator):
    ...
    def fit(self, data: DataNode, **kwargs):
        ...
        :return: selfdefault_dataset_name
        """
    ...
    def get_nsvd_importance(self, data: DataNode):
        X, y = data.data
...
class Regressor(BaseEstimator):
    ...
    def get_nsvd_importance(self, data: DataNode):
        ...
        if not os.path.exists(nsav_save_dir):
            os.makedirs(nsav_save_dir)
        joblib.dump(Gene_Contribution_Score, os.path.join(nsav_save_dir + "/Feature_Contribution_Score.pkl"))
        self.logger.info('Time cost for calculating NSAV: %.2f seconds' % (time.time() - start_time))
